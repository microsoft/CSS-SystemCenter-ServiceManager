...
if object_id (N'etl.SplitString', N'TF') is not null
    drop function etl.SplitString;
GO
create function etl.SplitString (@stringarray nvarchar(max))
returns @table table(string nvarchar(128) not null) as
begin
	...
	while @nextptr > 0
	   begin
		  ...
		  insert @table (string)
			 values (convert(nvarchar(128), substring(@stringarray, @ptr + 1, @length)))
		  select @ptr = @nextptr
	   end
...
if not exists(select * from INFORMATION_SCHEMA.TABLES where TABLE_NAME like 'CubePartition')
begin
	create table etl.CubePartition
	(
		PartitionId			int not null constraint PK_CubePartition primary key identity (1,1),
		...
		WatermarkBatchId 	int NULL
    )
end
go

-- FOR: UPGRADE
if not exists(select 'x' from sys.columns where name = 'WarehouseEntityName' and object_id = OBJECT_ID('etl.CubePartition'))
begin
    alter table etl.CubePartition
    add WarehouseEntityName nvarchar(512) null
end
go

if not exists(select 'x' from sys.columns where name = 'RangeStartDate' and object_id = OBJECT_ID('etl.CubePartition'))
begin
    alter table etl.CubePartition
    add RangeStartDate int null
end
go

if not exists(select 'x' from sys.columns where name = 'RangeEndDate' and object_id = OBJECT_ID('etl.CubePartition'))
begin
    alter table etl.CubePartition
    add RangeEndDate int null
end
go
...
END
GO
if not exists(select * from INFORMATION_SCHEMA.TABLES where TABLE_NAME like 'TranslationsWatermark')
begin
	create table dbo.TranslationsWatermark
	(
		WatermarkId		int NOT NULL primary key identity (1,1),
		WatermarkBatchId 	int NULL
	)
end
go

IF NOT EXISTS(SELECT * FROM dbo.TranslationsWatermark)
begin
insert into dbo.TranslationsWatermark values (0)
end
go

IF EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME like 'DeletionStatusDim') 
BEGIN

	IF EXISTS(SELECT 'x' from sys.columns where name = 'IsDeleted' and object_id = OBJECT_ID('dbo.DeletionStatusDim'))
	BEGIN
		DROP TABLE dbo.DeletionStatusDim

		CREATE TABLE dbo.DeletionStatusDim
		(
			DeletionStatusDimKey		INT NOT NULL PRIMARY KEY IDENTITY (0,1),		
			IsDeletedDisplayName NVARCHAR(20) NULL
		)
...
IF NOT EXISTS(SELECT * FROM dbo.DeletionStatusDim)
BEGIN
INSERT INTO dbo.DeletionStatusDim VALUES ('Active')
INSERT INTO dbo.DeletionStatusDim VALUES ('Deleted')
END
GO

   DECLARE @Alterstatement nvarchar(4000) ;
   DECLARE @tableviewname nvarchar(4000);
   DECLARE @tableviewcompletename nvarchar(4000);
   DECLARE table_cursor CURSOR FOR
        SELECT 'ALTER TABLE [' + TABLE_SCHEMA+'].['+TABLE_NAME +']
        ALTER COLUMN ' + COLUMN_NAME + ' decimal(19,0);',TABLE_NAME+'vw','['+TABLE_SCHEMA+'].['+TABLE_NAME+'vw]'
        FROM  INFORMATION_SCHEMA.COLUMNS
        WHERE TABLE_NAME LIKE '%LogicalDiskDim%'  AND TABLE_NAME NOT LIKE '%vw' AND
		COLUMN_NAME = 'Size'  AND DATA_TYPE != 'decimal';
      
    OPEN table_cursor     
    FETCH NEXT FROM table_cursor  
    INTO @Alterstatement, @tableviewname ,@tableviewcompletename
    WHILE @@FETCH_STATUS = 0
    BEGIN        
        EXEC sp_sqlexec @Alterstatement
		IF  EXISTS (SELECT 1 from INFORMATION_SCHEMA.VIEWS where TABLE_NAME = @tableviewname)
		BEGIN 
		EXEC sp_refreshview @tableviewcompletename
		END
        FETCH NEXT FROM table_cursor  
        INTO @Alterstatement, @tableviewname ,@tableviewcompletename
    END
	...
	DECLARE table_cursor CURSOR FOR
        ...
        ALTER COLUMN ' + COLUMN_NAME + ' decimal(19,0);',TABLE_NAME+'vw','['+TABLE_SCHEMA+'].['+TABLE_NAME+'vw]'
        FROM  INFORMATION_SCHEMA.COLUMNS
        WHERE TABLE_NAME LIKE '%OperatingSystemDim%'   AND TABLE_NAME NOT LIKE  '%vw' AND 
		COLUMN_NAME = 'PhysicalMemory'  AND DATA_TYPE != 'decimal';
...
if object_id (N'etl.PartitionSuffix', N'FN') is not null
    drop function etl.PartitionSuffix;
GO
create function [etl].[PartitionSuffix] (@MonthId tinyint)
returns nvarchar(64)
as
begin
	declare @Suffix nvarchar(64), @Month nchar(3)
    ...
    if (@Month = 'Jan') 
		set @Suffix = '_'+cast(DATEPART(year, getutcdate())+1 as nchar(4))+ '_' + @Month
	else 
		set @Suffix = '_'+cast(DATEPART(year, getutcdate()) as nchar(4))+ '_' + @Month
...
IF NOT EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'ReportDisplayStringGet')
	BEGIN
		EXECUTE ('CREATE PROCEDURE ReportDisplayStringGet AS RETURN 1')
	END
GO


ALTER PROCEDURE [dbo].[ReportDisplayStringGet]
...
BEGIN
  ...
  (
		SELECT DISTINCT		
			...
			NULL AS Parent,	
		    --mp.ManagementPackId,
		    --dsKORDim.ElementName,
		    --dsENUDim.ElementName,
		    --dsKORDim.SubElementName,
		    --dsENUDim.SubElementName,
		    --ISNULL(dsKORDim.ElementName, dsENUDim.ElementName) as ElementName,
		    ISNULL(dsKORDim.SubElementName, dsENUDim.SubElementName) as [S!1!N],			    
		    ISNULL(ISNULL(dsKORDim.DisplayName, dsENUDim.DisplayName), ISNULL(dsKORDim.SubElementName, dsENUDim.SubElementName))	AS [S!1!!ELEMENT]   
		FROM ManagementPackDim mp
		LEFT OUTER JOIN DisplayStringDim dsENUDim ON (dsENUDim.ManagementPackId = mp.ManagementPackId and dsENUDim.ElementName = @ReportSystemName and  dsENUDim.LanguageCode = 'ENU' )
		LEFT OUTER JOIN DisplayStringDim dsKORDim ON (dsKORDim.ManagementPackId = mp.ManagementPackId and dsKORDim.ElementName = dsENUDim.ElementName and  dsKORDim.SubElementName = dsENUDim.SubElementName and dsKORDim.LanguageCode = @LanguageCode )
		WHERE mp.MPName = @ManagementPackSystemName
		AND (dsKORDim.SubElementName <> 'NA' OR	dsENUDim.SubElementName <> 'NA')
		AND ((@Scope IS NULL) OR (dsKORDim.SubElementName like @Scope) OR (dsENUDim.SubElementName like @Scope))
...
IF NOT EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = N'GetLocalizedValuesForElement')
	BEGIN
		EXECUTE ('CREATE PROCEDURE dbo.[GetLocalizedValuesForElement] @ElementName nvarchar(512), @SubElementName nvarchar(512) AS RETURN 1')
	END
GO
ALTER PROCEDURE [dbo].[GetLocalizedValuesForElement] @BmeIds nvarchar(max)          
AS
BEGIN
select * from dbo.DisplayStringDimCrosstabvw where BaseManagedEntityId in (Select CAST(string as uniqueidentifier) From etl.SplitString(@BmeIds))
END
GO

IF NOT EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = N'GetPartitionsForCube')
	BEGIN
		EXECUTE ('CREATE PROCEDURE etl.[GetPartitionsForCube] @CubeName nvarchar(512) AS RETURN 1')
	END
GO
ALTER PROCEDURE [etl].[GetPartitionsForCube] @CubeName nvarchar(512)
...
BEGIN
 ...
 if (isnull(@CubeName, '') = '') 
 begin
  raiserror('Invalid cube name null',16,1)
  return -1
 end

 select cp.MeasureGroupName, cp.PartitionName, tp.RangeStartDate, tp.RangeEndDate, cp.WatermarkBatchId, tp.InsertedBatchId, tp.UpdatedBatchId  
 from etl.TablePartition tp
 join etl.CubePartition cp on (cp.PartitionName = tp.PartitionName)
 and cp.MeasureGroupName = MeasureGroupName
 and cp.CubeName = @CubeName
 
END
GO


IF NOT EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = N'GetPartitionsForFact')
	BEGIN
		EXECUTE ('CREATE PROCEDURE etl.[GetPartitionsForFact] @RelationalEntityName nvarchar(512) AS RETURN 1')
	END
GO
ALTER PROCEDURE [etl].[GetPartitionsForFact] @RelationalEntityName nvarchar(512)
...
BEGIN
 ...
 select PartitionName, RangeStartDate, RangeEndDate, InsertedBatchId, UpdatedBatchId  from etl.TablePartition where WarehouseEntityName = @RelationalEntityName
END
GO
if object_id ( 'etl.[UpdateCubePartitionEntries]', 'p' ) is not null 
    drop procedure etl.[UpdateCubePartitionEntries]
go
Create procedure etl.[UpdateCubePartitionEntries]
         @partitionsToManage XML
         
/*
 Updates the etl.CubePartition table by adding/deleting/updating partitions

 Parameters: 
 @partitionsToManage is the XML containing partitions to manage
  
 Usage
 exec etl.[UpdateCubePartitionEntries]
'<CubePartitionList>
	<PartitionEntry>
		<Cube>CubeToDelete</Cube>
		<MeasureGroup>MeasureGroupToDelete</MeasureGroup>
		<Partition>PartitionToDelete</Partition>
		<WarehouseEntityName>EntityName</WarehouseEntityName>
		<RangeStartDate>20110101</RangeStartDate>
		<RangeEndDate>20110101</RangeEndDate>
		<Action>Delete</Action>
	</PartitionEntry>
	<PartitionEntry>
		<Cube>CubeToAdd</Cube>
		<MeasureGroup>MeasureGroupToAdd</MeasureGroup>
		<Partition>PartitionToAdd</Partition>
		<WarehouseEntityName>EntityName</WarehouseEntityName>
		<RangeStartDate>20110101</RangeStartDate>
		<RangeEndDate>20110101</RangeEndDate>		
		<Action>Add</Action>
...
BEGIN TRY
	...
	DELETE cp
	FROM etl.CubePartition cp
	INNER JOIN @partitionsToManage.nodes('/CubePartitionList/PartitionEntry') AS del(P) ON
	cp.CubeName = del.p.value('Cube[1]', 'nvarchar(128)') AND
	cp.MeasureGroupName = del.p.value('MeasureGroup[1]', 'nvarchar(128)') AND
	cp.PartitionName = del.p.value('Partition[1]', 'nvarchar(128)') 
	where del.p.value('Action[1]', 'nvarchar(128)') = 'Delete'

	INSERT INTO etl.CubePartition(
			CubeName,
			MeasureGroupName,
			PartitionName,
			WarehouseEntityName,
	...
	SELECT  CubeName, MeasureGroupName, PartitionName, WarehouseEntityName, RangeStartDate, RangeEndDate
	FROM 
	(
		SELECT CubeEntry.CubeCol.value('Cube[1]', 'nvarchar(128)') AS CubeName,
		 CubeEntry.CubeCol.value('MeasureGroup[1]', 'nvarchar(128)') AS MeasureGroupName,
		 CubeEntry.CubeCol.value('Partition[1]', 'nvarchar(128)') AS PartitionName,
		 CubeEntry.CubeCol.value('WarehouseEntityName[1]', 'nvarchar(512)') AS WarehouseEntityName,
		 CubeEntry.CubeCol.value('RangeStartDate[1]', 'int') AS RangeStartDate,
		 CubeEntry.CubeCol.value('RangeEndDate[1]', 'int') AS RangeEndDate,
		 CubeEntry.CubeCol.value('Action[1]', 'nvarchar(128)') AS ActionName
		 FROM @partitionsToManage.nodes('/CubePartitionList/PartitionEntry') 
		 AS CubeEntry(CubeCol)
	)pta
	WHERE NOT EXISTS (SELECT * FROM etl.CubePartition WHERE CubeName = pta.CubeName and MeasureGroupName = pta.MeasureGroupName
	AND PartitionName = pta.PartitionName )
	AND pta.ActionName = 'Add'
	
	UPDATE cp
	SET RangeStartDate = upd.p.value('RangeStartDate[1]', 'int'), 
	    RangeEndDate = upd.p.value('RangeEndDate[1]', 'int'),
            WatermarkBatchId =
                 CASE WHEN (upd.p.value('RangeStartDate[1]', 'int') = 0) Then 0
                    ELSE cp.WatermarkBatchId 
                 END  

	from etl.CubePartition cp
	INNER JOIN 
	@partitionsToManage.nodes('/CubePartitionList/PartitionEntry') AS upd(P) ON
	CubeName = upd.p.value('Cube[1]', 'nvarchar(128)') AND
	MeasureGroupName = upd.p.value('MeasureGroup[1]', 'nvarchar(128)') AND
	PartitionName = upd.p.value('Partition[1]', 'nvarchar(128)') 	
	WHERE upd.p.value('Action[1]', 'nvarchar(128)') = 'Update'		
...
IF OBJECT_ID ( 'etl.UpdateCubeWatermark', 'P' ) IS NOT NULL 
    DROP PROCEDURE etl.UpdateCubeWatermark
GO

Create procedure etl.UpdateCubeWatermark
         @CubeWatermarks  XML
/*
 Updates cube watermarks in the etl.CubePartition table

 Parameters: 
 @CubeWatermarks is xml containing entries for each cube partition watermark to update
 
 Usage
 exec etl.[UpdateCubeWatermark]
'<CubeWatermarks>
	<PartitionEntry>
		<CubeName>A_CubeName</CubeName>
		<MeasureGroupName>A_MeasureGroupName</MeasureGroupName>
		<PartitionName>A_Partition</PartitionName>
...
BEGIN TRY
	...
	BEGIN TRANSACTION
	 ...
	 UPDATE etl.CubePartition SET WatermarkBatchId = CubeEntry.CubeCol.value('Watermark[1]', 'int')
	 FROM etl.CubePartition cp INNER JOIN 
	 @CubeWatermarks.nodes('/CubeWatermarks/PartitionEntry') AS CubeEntry(CubeCol) ON
	 cp.CubeName = CubeEntry.CubeCol.value('CubeName[1]', 'nvarchar(128)') AND
	 cp.MeasureGroupName = CubeEntry.CubeCol.value('MeasureGroupName[1]', 'nvarchar(128)') AND
	 cp.PartitionName = CubeEntry.CubeCol.value('PartitionName[1]', 'nvarchar(128)') 
...
IF OBJECT_ID ( 'etl.DropAllPartitionsForCube', 'P' ) IS NOT NULL 
    DROP PROCEDURE etl.DropAllPartitionsForCube
GO

create procedure etl.DropAllPartitionsForCube         
         @CubeName		nvarchar(128)
/*
 Drops all partition entry for the cube in the CubePartition table

 Parameters: 
 @CubeName is the name of the cube
 
 Usage
 exec etl.DropAllPartitionsForCube 'TestCube'
*/
as
begin
 set nocount on  
...
if object_id ( 'etl.UpdateTablePartitionBatchIdForFact', 'p' ) is not null 
    drop procedure etl.UpdateTablePartitionBatchIdForFact
go
Create procedure etl.UpdateTablePartitionBatchIdForFact
         @FactView nvarchar(128)
/*
 Updates the Inserted/Updated Batch Id for a fact in the etl.TablePartition table

 Parameters: 
 @FactView is the name of the fact view
 
 Usage
 exec etl.UpdateTablePartitionBatchIdForFact 'TestFactvw'
*/
...
DECLARE @Query nvarchar(max), @dateKey nvarchar(20)

 if (isnull(@FactView, '') = '') 
 begin
  raiserror('Invalid fact view null',16,1)
  return -1
 end


set @dateKey = 
(select COLUMN_NAME from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = @FactView 
and Column_Name in ('DateKey', 'MonthKey', 'WeekKey'))

--Get the data that will be moved via Group by month/date
--Then Update the Inserted/Updated batchid in etl.TablePartition
select @Query = 'select Cast(SubString(CAST(' + @dateKey + ' as varchar), 1, 6) as int) * 100 + 1 as ' + @dateKey + ', MAX(InsertedBatchId) as InsertedBatchId, MAX(UpdatedBatchId) as UpdatedBatchId
into #TempPartitionInfo from ' + @FactView + '_bulkstg group by SubString(CAST(' + @dateKey + ' as varchar), 1, 6)

update etl.TablePartition 
set UpdatedBatchId = data.UpdatedBatchId, InsertedBatchId = data.InsertedBatchId
from
(
select part.PartitionName, MAX(fact.InsertedBatchId) as InsertedBatchId, MAX(fact.UpdatedBatchId) as UpdatedBatchId from etl.TablePartition part
inner join #TempPartitionInfo fact on (isnull(part.RangeStartDate,0) <= fact.' + @dateKey + ' and isnull(part.RangeEndDate, 99999999) >= fact.' + @dateKey + ')
where WarehouseEntityName = SUBSTRING(''' + @FactView + ''', 1, len(''' + @FactView + ''')-2)
group by part.PartitionName
) as data
inner join etl.TablePartition tp on (data.PartitionName = tp.PartitionName)
where tp.partitionname = data.partitionname
...
--Drop temp table
IF OBJECT_ID('tempdb..#TempPartitionInfo') IS NOT NULL
BEGIN
	DROP TABLE #TempPartitionInfo
END
...
if object_id ( 'etl.UpdateTablePartitionBatchIdForNonFact', 'p' ) is not null 
    drop procedure etl.UpdateTablePartitionBatchIdForNonFact
go
Create procedure etl.UpdateTablePartitionBatchIdForNonFact
         @EntityName nvarchar(128)
/*
 Updates the Inserted/Updated Batch Id for a dimension\outrigger in the etl.TablePartition table

 Unlike what we do for a fact, there is no need to group by partition name since only facts will have multiple partitions.
 Dimensions/Outriggers will only have one partition

 Parameters: 
 @EntityName is the name of the  entity
 
 Usage
 exec etl.UpdateTablePartitionBatchIdForNonFact 'ComputerDim'
*/
...
DECLARE @Query nvarchar(max)
 ...
 begin
  raiserror('Invalid entity view null',16,1)
  return -1
 end
...
DECLARE @maxInsertedBatch int, @maxUpdatedBatch int, @currentInsertedBatch int, @currentUpdatedBatch int

SELECT @maxInsertedBatch = ISNULL(MAX(InsertedBatchId), 0), @maxUpdatedBatch = ISNULL(MAX(UpdatedBatchId), 0)
FROM ' + @EntityName + '_bulkstg

SELECT @currentInsertedBatch = ISNULL(InsertedBatchId, 0),
@currentUpdatedBatch = ISNULL(UpdatedBatchId, 0)
FROM etl.TablePartition
WHERE WarehouseEntityName = ''' + @EntityName + '''

IF @maxInsertedBatch < @currentInsertedBatch
BEGIN
	SET @maxInsertedBatch = @currentInsertedBatch	
END

IF @maxUpdatedBatch < @currentUpdatedBatch
BEGIN
	SET @maxUpdatedBatch = @currentUpdatedBatch
END

UPDATE etl.TablePartition
SET InsertedBatchId = @maxInsertedBatch, UpdatedBatchId = @maxUpdatedBatch
WHERE WarehouseEntityName = ''' + @EntityName + '''
...
IF object_id ( 'dbo.GetWatermarksForCubeTranslations', 'p' ) is not null 
    DROP PROCEDURE dbo.GetWatermarksForCubeTranslations
GO
CREATE PROCEDURE [dbo].GetWatermarksForCubeTranslations
AS
BEGIN
...
DECLARE @maxDisplayStringDimWM INT,
		@currentTranslationsWM INT,
		@insertedBatchId INT,
		@updatedBatchId INT
      
SET @updatedBatchId = (SELECT UpdatedBatchId FROM etl.TablePartition WHERE PartitionName = 'DisplayStringDim');
SET @insertedBatchId = (SELECT InsertedBatchId FROM etl.TablePartition WHERE PartitionName = 'DisplayStringDim');

SET @maxDisplayStringDimWM = 
    CASE 
        WHEN  (@insertedBatchId >= @updatedBatchId) THEN @insertedBatchId
        WHEN (@insertedBatchId < @updatedBatchId) THEN @updatedBatchId
    END
	    
SET @currentTranslationsWM = (select WaterMarkBatchId from dbo.TranslationsWatermark);


SELECT @maxDisplayStringDimWM as DisplayStringDimWM, @currentTranslationsWM as TranslationsWM
...
if object_id ( 'dbo.SetTranslationsWatermark', 'p' ) is not null 
    drop procedure dbo.SetTranslationsWatermark
go
create procedure dbo.SetTranslationsWatermark
@WaterMarkValue	int		
as
begin
	set nocount on 	

	update dbo.TranslationsWatermark set WaterMarkBatchId = @WaterMarkValue

	set nocount off
end	
go

if object_id ( 'etl.[GetCubePartitionsToManage]', 'p' ) is not null 
    drop procedure etl.[GetCubePartitionsToManage]
go
Create procedure etl.[GetCubePartitionsToManage]
         @CubesToManage XML         
/*
 Determines which partitions need to be added and deleted from the etl.CubePartition table

 Parameters: 
 @CubesToManage is the XML containing cubes to manage
 
 Usage
 exec etl.[GetCubePartitionsToManage]
'<CubeList>
	<CubeEntry>
		<CubeName>ComputerCubeWithKpi</CubeName>
		<DateKeyBoundary>20110101</DateKeyBoundary>
	</CubeEntry>
	<CubeEntry>
		<CubeName>DimSubstitution01TestCube</CubeName>
		<DateKeyBoundary>20110101</DateKeyBoundary>
	</CubeEntry>	
</CubeList>'
*/
...
BEGIN TRY
	...
	( 
	    ...
	    WarehouseEntityName nvarchar(512),
	    PartitionAction nvarchar(128)
	)	  
	 
	DECLARE cube_cursor CURSOR FOR
		SELECT CubeEntry.CubeCol.value('CubeName[1]', 'nvarchar(128)') AS CubeName,
			   CubeEntry.CubeCol.value('DateKeyBoundary[1]', 'nvarchar(128)') AS DateKeyBoundary
		 FROM @CubesToManage.nodes('/CubeList/CubeEntry') 
		 AS CubeEntry(CubeCol);

	OPEN cube_cursor;

	DECLARE @cubeName nvarchar(128),
			@dateKeyBoundary int


	FETCH NEXT FROM cube_cursor INTO @cubeName, @dateKeyBoundary
	WHILE @@FETCH_STATUS = 0 BEGIN

	--Query for which partitions to add to etl.CubePartition
	INSERT INTO #tempPartitions
	SELECT DISTINCT @cubeName as CubeName, tp.RangeEndDate, tp.RangeStartDate, tp.PartitionName, tp.WarehouseEntityName, 'Add' as PartitionAction
	FROM etl.TablePartition  tp
	WHERE NOT EXISTS
	(
	SELECT * FROM etl.CubePartition cp WHERE (tp.WarehouseEntityName = cp.WarehouseEntityName)
	AND (tp.PartitionName = cp.PartitionName)
	AND cp.CubeName = @cubeName
	)
	AND tp.WarehouseEntityName in 
	(
	SELECT DISTINCT WarehouseEntityName FROM etl.CubePartition WHERE CubeName = @cubeName
	)
	AND ((tp.RangeEndDate is not NULL AND tp.RangeEndDate >= @dateKeyBoundary) OR (tp.RangeEndDate is NULL))


	--Query for which partitions to delete from etl.CubePartition
	INSERT INTO #tempPartitions
	SELECT DISTINCT @cubeName as CubeName, NULL as RangeEndDate, NULL as RangeStartDate, cp.PartitionName, cp.WarehouseEntityName, 'Delete' as PartitionAction 
	FROM etl.CubePartition cp
	WHERE NOT EXISTS
	(
	SELECT * FROM etl.TablePartition tp WHERE (tp.WarehouseEntityName = cp.WarehouseEntityName)
	AND (tp.PartitionName = cp.PartitionName)
	)
	AND cp.CubeName = @cubeName

	--Add partitions to delete that are below the datekeyboundary cutoff
	IF @dateKeyBoundary != -1
	BEGIN

	INSERT INTO #tempPartitions
	SELECT DISTINCT @cubeName as CubeName, NULL as RangeEndDate, NULL as RangeStartDate, cp.PartitionName, cp.WarehouseEntityName, 'Delete' as PartitionAction 
	FROM etl.CubePartition cp
	WHERE cp.RangeEndDate is not NULL AND cp.RangeEndDate != 0 AND cp.RangeEndDate < @dateKeyBoundary AND cp.CubeName = @cubeName

	END

	--Query for which partitions to update from etl.CubePartition
	INSERT INTO #tempPartitions
	SELECT DISTINCT @cubeName as CubeName, tp.RangeEndDate as RangeEndDate, tp.RangeStartDate as RangeStartDate, cp.PartitionName, cp.WarehouseEntityName, 'Update' as PartitionAction 
	FROM etl.TablePartition tp inner join etl.CubePartition cp 
	ON tp.PartitionName = cp.PartitionName and tp.WarehouseEntityName = cp.WarehouseEntityName 
	WHERE (ISNULL(tp.RangeStartDate, 0) != cp.RangeStartDate or tp.RangeEndDate != cp.RangeEndDate)
	AND cp.CubeName = @cubeName
	  ...
	  END
	CLOSE cube_cursor;
	DEALLOCATE cube_cursor;
	 
	 SELECT * FROM #tempPartitions
...
--
-- Sproc Name: p_DataWarehouseTypeSpaceDataSelect
-- Description: All type information for DataWarehouse TypeSpaceData.
-- Caller: SDK
--
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_NAME = 'p_DataWarehouseTypeSpaceDataSelect' AND ROUTINE_TYPE = 'PROCEDURE')
    DROP PROCEDURE dbo.p_DataWarehouseTypeSpaceDataSelect;
GO

CREATE PROCEDURE dbo.p_DataWarehouseTypeSpaceDataSelect
...
BEGIN
    ...
    SELECT
        S.TABLE_NAME AS [ObjectName],
        S.TABLE_SCHEMA AS [Owner],
        S.COLUMN_NAME AS [Name],
        S.DATA_TYPE AS [Type],
        S.CHARACTER_OCTET_LENGTH AS [Size],
        S.CHARACTER_MAXIMUM_LENGTH AS [Length],
        S.ORDINAL_POSITION AS [Ordinal],
        CASE 
            WHEN S.IS_NULLABLE = 'YES'
            THEN CAST(1 AS BIT)
            ELSE CAST(0 AS BIT)
            END AS [IsNullable]
    FROM INFORMATION_SCHEMA.COLUMNS AS S
    WHERE S.TABLE_SCHEMA COLLATE DATABASE_DEFAULT = N'dbo'
        AND S.TABLE_NAME COLLATE DATABASE_DEFAULT IN
            (@TableName);
...
--
-- Sproc Name: p_DataWarehouseOutriggerTypeSpaceDataSelect
-- Description: All type information for DataWarehouse TypeSpaceData.
-- Caller: SDK
--
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_NAME = 'p_DataWarehouseOutriggerTypeSpaceDataSelect' AND ROUTINE_TYPE = 'PROCEDURE')
    DROP PROCEDURE dbo.p_DataWarehouseOutriggerTypeSpaceDataSelect;
GO

CREATE PROCEDURE dbo.p_DataWarehouseOutriggerTypeSpaceDataSelect
(@TableName Varchar(255))
AS
BEGIN
    SET NOCOUNT ON;
...
FROM (          -- 3 years = 1mi * 60mis * 24hrs * 365days * 3yrs = 1576800
                SELECT 'DWMaintenance.Grooming' AS ConfigurationFilter, '1576800' AS ConfiguredValue, 'RetentionPeriodInMinutes.Default' AS ConfigurationPath, 'Int32' AS ConfiguredValueType
    ...
    -- 3 months = 1mi * 60mis * 24hrs * 30days * 3mons = 129600 for Chargeback daily facts
    UNION ALL   SELECT 'DWMaintenance.Grooming.RetentionPeriodInMinutes', '129600', 'PricesheetDetailDailyFact', 'Int32'
    UNION ALL   SELECT 'DWMaintenance.Grooming.RetentionPeriodInMinutes', '129600', 'InfraChargebackDailyFact', 'Int32'

    UNION ALL   SELECT 'DWMaintenance.Grooming.GroomingStoredProcedure', 'EXEC etl.DropPartition @WarehouseEntityId=@WarehouseEntityId, @WarehouseEntityType=@WarehouseEntityType, @EntityGuid=@EntityGuid, @PartitionId=@PartitionId, @GroomActiveRelationship=0', 'GroomingProcedure.Default', 'Int32'
) AS A
...
UPDATE Cfg SET
        ConfiguredValue         = tempCfg.ConfiguredValue,
        ConfiguredValueType     = tempCfg.ConfiguredValueType,
        ModifiedDateTime        = GETUTCDATE()
FROM etl.Configuration Cfg
INNER JOIN #TempConfiguration tempCfg ON
        Cfg.ConfigurationFilter = tempCfg.ConfigurationFilter
    AND Cfg.ConfigurationPath   = tempCfg.ConfigurationPath
WHERE   Cfg.ConfiguredValue     <> tempCfg.ConfiguredValue
    OR  Cfg.ConfiguredValueType <> tempCfg.ConfiguredValueType
...
GO
---------------------------------------------------------------------------------
--  (c) Copyright 2006-2009 Microsoft Corporation, All Rights Reserved         --
--  Proprietary and confidential to Microsoft Corporation                      --
--                                                                             --
--  File:      DWRole.sql                                                      --
--                                                                             --
--  Contents: Creates role for report useraccount with appropriate permissions --
---------------------------------------------------------------------------------
...
IF NOT EXISTS (SELECT * FROM sysusers WHERE [name] = N'reportuser')
	EXEC sp_addrole N'reportuser'
GO

-- language string access
GRANT EXECUTE ON dbo.ReportDisplayStringGet TO reportuser
GO
